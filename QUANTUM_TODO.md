...
- [ ] Identify Rust migration hot paths
  - **Candidates**: `analyzePrompt()`, `estimateTokens()`, `globSearch()`, `grepSearch()` — all already ported to Rust
  - **Missing**: benchmark framework to validate speedup (Criterion dep exists in `Cargo.toml` but no benchmarks written)
...
### 5.1 Router ↔ QueryEngine Integration (NEW — CRITICAL)
- [ ] Call `router.route(prompt, cwd)` before each LLM API call in `QueryEngine.ts`
...
### 5.4 Chat Mode
- [ ] Implement lightweight conversation path (minimal context, no file loading)
- [ ] Create minimal tool activation (`ToolPolicyManager.ACTIVATION_MINIMAL`)
- [ ] Add fast response optimization (skip memory loading, minimal system prompt)
- [ ] Implement chat → plan escalation (detect complexity > simple → suggest mode switch)
