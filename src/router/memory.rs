...
mod tests {
    ...
    fn test_get_memory_hint() {
        assert_eq!(
            get_memory_hint(MemoryPolicy::None),
            "No memory loading needed"
        );
        assert_eq!(
            get_memory_hint(MemoryPolicy::Recent),
            "Load recently modified files"
        );
        assert_eq!(
            get_memory_hint(MemoryPolicy::Relevant),
            "Load files relevant to current task"
        );
        assert_eq!(
            get_memory_hint(MemoryPolicy::Full),
            "Load all recent project context"
        );
    }
}
